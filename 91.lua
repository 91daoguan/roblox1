...
-- UICorner 工具函数
local function applyCorner(uiObject, radius)
...
local function createRevolutionUI()
    ...
    -- 旗帜按钮区 (用 ImageButton 替代 ImageLabel)
    local ToggleBtn = ScreenGui:FindFirstChild("FlagToggle") or Instance.new("ImageButton")
    ToggleBtn.Name = "FlagToggle"
    ToggleBtn.Size = UDim2.new(0, 120, 0, 180)
    ToggleBtn.Position = UDim2.new(1, -130, 0, 20)
    ToggleBtn.BackgroundTransparency = 1
    ToggleBtn.Image = "rbxassetid://123456" -- 请替换为实际资产ID
    ToggleBtn.Parent = ScreenGui
    ToggleBtn.ZIndex = 5
    ToggleBtn.Active = true -- 确保可交互
    ...
    ToggleBtn.MouseButton1Click:Connect(function()
        ...
        local targetPos = isUIVisible and UDim2.new(0.1,0, 0.05,0) or UDim2.new(0.1,0, -1,0)
        TweenService:Create(MainFrame,TWEEN_INFO,{Position = targetPos}):Play()
    end)

    -- 初始化入场动画
    TweenService:Create(MainFrame, TWEEN_INFO, {Position = UDim2.new(0.1, 0, 0.05, 0)}):Play()
end
...
local function updateStatusText()
    local StatusLabel
    local MainFrame = ScreenGui:FindFirstChild("IndependenceHallFrame")
    if MainFrame then
...
local function updateClimbState(enabled)
    ...
    if MainFrame then
        ...
        if ButtonFrame then
            local ClimbBtn = ButtonFrame:FindFirstChild("ClimbBtn")
            if ClimbBtn then
...
local function updateFlyState(enabled)
    ...
    if MainFrame then
        local ButtonFrame = MainFrame:FindFirstChild("ButtonFrame")
        if ButtonFrame then
            local FlyBtn = ButtonFrame:FindFirstChild("FlyBtn")
            if FlyBtn then
...
local function updateNoclipState(enabled)
    ...
    if MainFrame then
        local ButtonFrame = MainFrame:FindFirstChild("ButtonFrame")
        if ButtonFrame then
            local NoclipBtn = ButtonFrame:FindFirstChild("NoclipBtn")
            if NoclipBtn then
